...
def maybe_qdrant_upsert(records: List[Dict[str, Any]]) -> None:
    ...
    Controlled by env: QDRANT_UPSERT=1
    """
    if os.getenv("QDRANT_UPSERT", "") != "1":
        print("[INFO] Skipping Qdrant upsert (QDRANT_UPSERT!=1)")
        return
    ...
    print(f"[OK] Upserted {len(points)} vectors to Qdrant::{collection}")
...
def main() -> int:
    ...
    for p in files:
        ...
        for i, ch in enumerate(chunks):
            ...
            manifest.append(
                {"file": p.as_posix(), "chunk": i, "id": doc_id, "len": len(ch)}
            )
    ...
    if mode == "dry":
        print(
            "[INFO] DRY RUN: wrote chunk manifest only "
            "(set OPENAI_API_KEY or EMBED_MODE=local to embed)"
        )
        return 0
